...
| Nginx (pronouced engine-x) | Nginx is a web server on which you can host a website and manage load |
...
A Reindeer wiil be displayed on the bottom left of the page. It will tell the user what to type in the searchbar, or warn the user if what they typed in is in a valid format (For more info go see the [error messages](#3c-error-messages)).
In order to ease the use of the website by users, we will have cookies in which we will store the adress typed, that we will be activited only if the user agrees to it. Thanks to this, the user will not have to type his adress each time is going on the website.
The website will be accessible through any web browser. 

### **Back-end**

The website will be deployed on an nginx server. The nginx server will take care of redirecting the adress type to the database, process the calculations to get the remaining time until Christmas and sending back the info to the front-end.

### **Database**

For our database, we will use the data from [geofabrik](http://download.geofabrik.de). This dataset is quite heavy but it allows us to have a great precision, and it has also regular updates. The data will be stored in a PostgreSQL database.
In the case where this database is not fast enough or if is manipulation is too complicated, we could use data from this [dataset](http://www.geonames.org). This dataset is less precise because it only take in account the city and not the complete adress, but it is less heavier and easier to manipulate than the data from openstreetmap.
...
| "I can not find you. It seems you did not activite your localisation" | The user tried to use the geolocalisation feature but he did not allow his browser to have access to his localisation |
| | |
